...
const PostFeed:React.FC<PostFeedProps> = ({initialPosts, subredditName}) => {
  ...
  const {data,fetchNextPage,isFetchingNextPage}=useInfiniteQuery(
    ['infinite-query'],
    async({pageParam=1})=>{
        const query= `/api/posts?limit${INFINITE_SCROLLING_PAGINATION_RESULTS}&page=${pageParam}`+(!!subredditName?`&subreddit=${subredditName}`:'')
    ...
    <ul className='flex flex-col col-span-2 space-y-6'>
        {posts.map((post,index)=>{
            ...
            if(index===posts.length-1){
                ...
                return <Post commentAmt={post.comments.length} subredditName={post.subreddit.name} post={post}/>
            }
        })}
    </ul>
  )
